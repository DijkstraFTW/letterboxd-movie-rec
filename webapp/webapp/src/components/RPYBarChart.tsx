...
// @ts-expect-error
const RPYBarChart = ({data, x_field, y_field}) => {
            ...
            return (
                <div className="inline-block bg-gray-800 p-1.5 rounded-sm">
                    <span className="flex flex-row items-center justify-center text-white font-bold">
                        {`${payload[0].value.toLocaleString(0)}`}</span>
                    <span className="flex flex-row items-center text-gray-500">{`watched in ${label}`}</span>
                </div>
            );
        }
    };
